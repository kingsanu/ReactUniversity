...
export function GenerationContextForm({
  ...
  const formFields: Record<string, Array<{ name: string; label: string; type: string; required?: boolean; placeholder?: string; tooltip?: string }>> = {
    summary: [
      {
        name: 'current_role',
        label: 'Current Role',
      ...
      },
      {
        ...
        type: 'textarea',
        placeholder: 'e.g., Led team of 5 engineers, increased performance by 40%',
    ...
    bullets: [
      ...
      },
      {
        ...
        type: 'textarea',
        required: true,
        placeholder: 'What did you do? e.g., Developed REST APIs, managed database migrations',
  ...
  const toneOptions = ['Professional', 'Enthusiastic', 'Confident', 'Analytical', 'Creative'];
    ...
    <div className="space-y-4">
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
        <p className="text-xs text-blue-900">
          <strong>💡 Tip:</strong> The more details you provide, the better the generated content will be.
        </p>
      </div>
      ...
      {fields.map((fieldConfig) => (
        <div key={fieldConfig.name} className="space-y-1.5">
          <div className="flex items-center gap-2">
            <label htmlFor={fieldConfig.name} className="text-sm font-medium text-foreground">
              {fieldConfig.label}
            ...
            {fieldConfig.tooltip && (
              <button
                type="button"
                onClick={() =>
                  setExpandedTip(expandedTip === fieldConfig.name ? null : fieldConfig.name)
                }
      ...
      <div className="flex items-start gap-2 p-3 bg-muted/30 border border-border rounded-lg">
        ...
        <p className="text-xs text-muted-foreground">
          Your information is used only to generate content and is not stored. The generated content will be optimized for ATS systems.
        </p>
      </div>
    </div>
  );
