...
import { testPCAAPIStructure, testPCAAPIWithRealUser } from '../../utils/testPCAAPI';
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-lg shadow-sm border p-6">
          ...
          <div className="space-y-4 mb-6">
            ...
            >
              {isLoading ? 'Testing...' : `Test with Current User ${user?.id ? `(${user.id})` : '(Not logged in)'}`}
            </button>
          </div>
          ...
          {user?.id && (
            <div className="mb-4 p-3 bg-blue-50 rounded-lg">
              <p className="text-sm text-blue-800">
                Current User: <code className="bg-blue-100 px-1 rounded">{user.id}</code>
              </p>
            </div>
          )}
          ...
          <div className="bg-gray-900 text-green-400 p-4 rounded-lg font-mono text-sm overflow-auto max-h-96">
            <pre className="whitespace-pre-wrap">
              {testResults || 'Click a button above to run tests...'}
            </pre>
          </div>
          ...
          <div className="mt-6">
            ...
            <ul className="text-sm text-gray-600 space-y-1">
              <li>• <code>GET /api/pcaapi/evaluations</code> - Get all PCA evaluations</li>
              <li>• <code>POST /api/pcaapi/get-result</code> - Get PCA result by UserId</li>
              <li>• <code>checkPCAStatus()</code> - Custom function to determine status</li>
            </ul>
          </div>
