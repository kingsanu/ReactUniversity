...
export default function GlassDemo() {
  ...
  return (
    <div className="min-h-[100dvh] w-full relative font-sans selection:bg-black selection:text-white pb-20">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none transition-colors duration-1000 bg-[#e7e5e1]">
        {activeVariant === 0 && (
          <div className="absolute inset-0 opacity-100 transition-opacity duration-1000">
            {/* Minimal Background (like a stylized blurred rock/earth in reference image) */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120vw] h-[120vw] md:w-[80vw] md:h-[80vw] bg-[#cfccc2] rounded-full blur-[140px] opacity-90" />
            <div className="absolute top-1/2 left-[30%] -translate-x-1/2 -translate-y-[40%] w-[60vw] h-[80vw] bg-[#d1cbbd] rotate-12 blur-[100px] opacity-80" />
            <div className="absolute bottom-[-20%] right-[-10%] w-[50vw] h-[50vw] bg-[#e1ddcf] rounded-full blur-[100px] opacity-90" />
          </div>
        )}
        {activeVariant === 1 && (
      ...
      {/* Navigation / Header */}
      <div className="relative z-30 flex w-full max-w-screen-2xl mx-auto items-center justify-between p-6 md:p-8">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full bg-stone-900 flex items-center justify-center">
            <div className="w-3 h-3 rounded-full bg-white opacity-80" />
          </div>
          <h1 className={`text-lg font-medium tracking-tight ${activeVariant === 1 ? 'text-white' : 'text-stone-900'}`}>
            GreenMotive Inspired
          </h1>
        </div>
        
        <div className="flex gap-1.5 p-1.5 bg-black/[0.03] dark:bg-white/[0.05] backdrop-blur-xl rounded-full border border-black/[0.05] dark:border-white/[0.05]">
          {VARIANTS.map((variant, i) => (
            <button
              key={variant}
              onClick={() => setActiveVariant(i)}
              className={`px-4 py-2 md:px-6 rounded-full text-sm font-medium transition-all duration-300 ${
                activeVariant === i
                  ? activeVariant === 1
        ...
        <div className={`hidden lg:flex items-center gap-2 text-sm font-medium ${activeVariant === 1 ? 'text-white/80' : 'text-stone-900/80'}`}>
      ...
      {/* Main Content Area */}
      <div className="relative z-20 flex flex-col items-center justify-center w-full max-w-[1400px] mx-auto px-6 py-10 md:py-20 mt-8 md:mt-12">
        
        {/* Title corresponding to variant */}
        <AnimatePresence mode="wait">
          <motion.h2
            ...
            transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }} // smooth cubic-bezier
            className={`text-6xl md:text-8xl lg:text-[8rem] font-medium tracking-tighter mb-16 md:mb-28 text-center leading-[0.9] ${
        ...
        {/* 3 Column Grid for Cards (Bento/Gallery Paradigm) */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-14 w-full max-w-6xl place-items-center perspective-[1000px]">
...
function GlassCard({ card, variant, index }: { card: any; variant: number; index: number }) {
  // We use Framer Motion useMotionValue for the magnetic hover effect
  const [isHovered, setIsHovered] = useState(false);

  // High-End Glassmorphism Styles (Liquid Glass Refraction and Drop Shadows)
  const getVariantStyles = () => {
    switch (variant) {
      case 0: // Minimal Frost (Exact reference style)
        return {
          // Inner 1px border is critical: box-shadow inset + border
  ...
  return (
    <motion.div
      ...
      className={`
        ...
        w-full aspect-[2.9/4.5] sm:max-w-[340px] rounded-[2.5rem] p-8 md:p-10
        transition-all duration-500 overflow-hidden cursor-pointer
        border border-solid ${styles?.container}
      `}
    >
      {/* Shine effect overlay for extra materiality */}
      <div 
      ...
      <motion.div 
        ...
        className={`px-5 py-2 rounded-full text-[11px] font-semibold tracking-widest uppercase mb-12 ${styles?.topLabelContainer}`}
      >
        {card.label}
      </motion.div>

      {/* Center Icon (Perpetual Micro-Interaction) */}
      <motion.div
        animate={{ 
      ...
      >
        {card.icon}
        {/* Abstract plus or dot just to add detail as in the reference image */}
      ...
      >
        <h3 className="text-[1.3rem] md:text-2xl font-medium tracking-tight whitespace-pre-line leading-[1.1] mb-2">
          {card.title}
        </h3>
        
        {/* Tiny decorative line/arrow that appears on hover */}
        <motion.div 
