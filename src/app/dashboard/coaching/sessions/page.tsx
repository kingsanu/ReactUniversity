...
export default function SessionsPage() {
        ...
        const formattedSessions = rawSessions
          .map((session: any) => {
            ...
            return {
              ...
              time,
              duration,
              studentName: session.studentName || session.userName || student.name || student.fullName || "Student",
              studentAvatar: session.studentAvatar || session.userAvatar || student.image || student.avatar,
              studentId: session.studentId || session.userId || student.id || student._id, // Ensure we have student ID
  ...
  useEffect(() => {
    const fetchSlots = async () => {
      if (!isRescheduleOpen || !selectedSession || !rescheduleDate || !user) return;
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            >
              ...
              <div className={cn("absolute -right-6 -top-6 h-32 w-32 rounded-full opacity-10 blur-2xl transition-transform group-hover:scale-150 bg-gradient-to-br", stat.gradient)} />
              
              <div className="relative z-10 flex flex-col justify-between h-full gap-4">
                 <div className="flex justify-between items-start">
                    <div className={cn("h-12 w-12 rounded-2xl flex items-center justify-center shadow-lg text-white bg-gradient-to-br", stat.gradient, stat.shadow)}>
                       <stat.icon className="h-6 w-6" strokeWidth={2} />
                    </div>
                    {/* Optional Trend Badge (Visual only) */}
                    <div className={cn("px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide bg-white border shadow-sm", stat.textGradient.replace("text-", "text-").replace("600", "700"))}>
                       Details
                    </div>
                 </div>

                 <div>
                    <h3 className={cn("text-4xl font-extrabold tracking-tight mt-2 bg-clip-text text-transparent bg-gradient-to-br", stat.gradient)}>
                       {stat.value}
                    </h3>
        ...
        <div className="space-y-6">
          {isLoading ? (
             <div className="flex flex-col items-center justify-center py-20 text-gray-400 space-y-4">
                ...
                <p className="font-medium animate-pulse">Loading your sessions...</p>
             </div>
             ...
             <div className="text-center py-20 sm:py-32 bg-white/60 backdrop-blur-xl rounded-[2rem] border-2 border-dashed border-gray-200/60 flex flex-col items-center justify-center">
                 ...
                 <p className="text-gray-500 max-w-sm mx-auto">
                   {searchQuery ? "Try adjusting your filters or search query." : "Looks like you haven't scheduled any sessions yet."}
                 </p>
                 {searchQuery && (
                   <Button variant="link" onClick={() => {setSearchQuery(""); setStatusFilter(null);}} className="mt-4 text-blue-600">
                     Clear filters
                   </Button>
                 )}
             </div>
                         ...
                         >
                           <div className="relative bg-white/40 backdrop-blur-md p-6 rounded-3xl border border-white/60 shadow-sm hover:shadow-xl hover:shadow-blue-900/5 transition-all duration-300 overflow-hidden hover:bg-white/60">
                             ...
                             <div className={cn(
                               ...
                               session.status === "cancelled" ? "bg-gradient-to-b from-red-400 to-red-600" :
                               session.status === "completed" ? "bg-gradient-to-b from-green-400 to-green-600" :
                             ...
                             <div className="flex flex-col lg:flex-row gap-6 items-start lg:items-center pl-2">
                                ...
                                >
                                   <div className="relative">
                                     <div className={cn("absolute inset-0 rounded-full blur-sm opacity-20", 
                                   ...
                                   <div className="flex-1 min-w-0 space-y-1">
                                      ...
                                      <div className="flex items-center gap-2">
                                        <Badge variant="secondary" className="bg-white/80 border text-gray-500 font-medium px-2.5 py-0.5 shadow-sm">
                                ...
                                <div className="flex items-center gap-3 w-full lg:w-auto justify-end mt-4 lg:mt-0 pt-5 lg:pt-0 border-t lg:border-t-0 border-gray-100/50">
                                   ...
                                   <DropdownMenu>
                                      <DropdownMenuTrigger asChild>
                                         <Button variant="ghost" size="icon" className="h-11 w-11 rounded-2xl hover:bg-white hover:shadow-md text-gray-400 hover:text-gray-900 transition-all">
                                            <MoreVertical className="h-5 w-5" />
                                         </Button>
                                      </DropdownMenuTrigger>
                                      <DropdownMenuContent align="end" className="w-48 rounded-xl p-1.5 shadow-xl border-gray-100">
                                         <DropdownMenuItem onClick={() => handleViewProfile(session.studentId)} className="rounded-lg p-2.5 font-medium cursor-pointer">
                                            <User className="mr-2 h-4 w-4 text-gray-400" /> Profile
                                         </DropdownMenuItem>
                                         {(session.status === "confirmed" || session.status === "rescheduled") && (
                                            <>
                                              <DropdownMenuSeparator className="bg-gray-100 my-1" />
                                              <DropdownMenuItem onClick={() => handleRescheduleClick(session)} className="rounded-lg p-2.5 font-medium cursor-pointer focus:bg-orange-50 focus:text-orange-700 text-gray-700">
                                                 <CalendarDays className="mr-2 h-4 w-4 text-orange-400" /> Reschedule
                                              </DropdownMenuItem>
                                              <DropdownMenuItem onClick={() => { setSelectedSession(session); setIsConfirmCancelOpen(true); }} className="rounded-lg p-2.5 font-medium cursor-pointer focus:bg-red-50 focus:text-red-700 text-red-600">
        ...
        <DialogContent className="max-w-2xl rounded-3xl p-0 overflow-hidden bg-white border-none shadow-2xl">
          <DialogHeader className="p-6 pb-2">
            ...
            <DialogDescription className="ml-14 text-base">
               Private notes for your session with <span className="font-semibold text-gray-900">{selectedSession?.studentName}</span>
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="p-6 pt-2 bg-gray-50/50">
            <Button onClick={() => setIsNotesOpen(false)} className="w-full sm:w-auto h-12 rounded-xl bg-gray-900 text-white font-semibold">
               Close
            </Button>
          </DialogFooter>
        </DialogContent>
        ...
        <DialogContent className="max-w-md rounded-3xl p-0 overflow-hidden bg-white border-none shadow-2xl">
           <div className="p-8 text-center flex flex-col items-center">
              ...
              <p className="text-gray-500 text-center mb-8 leading-relaxed">
                 Are you sure you want to cancel the session with <span className="font-semibold text-gray-900">{selectedSession?.studentName}</span>? This action cannot be undone.
              </p>
                 ...
                 <Button 
                   ...
                   onClick={async () => {
                     try {
                        ...
                        await cancelSession(selectedSession.id, "Cancelled by coach");
                  ...
                  <div className="p-6 border-b border-gray-200/50">
                     <div className="flex items-center gap-3">
                        <Avatar className="h-10 w-10 border-2 border-white shadow-sm">
                          <AvatarImage src={selectedSession?.studentImage || selectedSession?.studentAvatar} />
                        ...
                        <div className="flex-1 min-w-0">
                           ...
                           <p className="text-xs text-gray-500 truncate">{selectedSession?.topic?.toUpperCase()}</p>
                        </div>
                     </div>
                  </div>
