...
export default function TransactionsPage() {
  ...
  const invoiceCount = transactions.filter((t) => t.status === "completed").length;
  
  const activeMethod = transactions.length > 0 ? transactions[0].method : "No active method";
    ...
    <div className="space-y-8 p-8 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-3">
            <Button variant="outline" className="h-10 gap-2 rounded-xl" onClick={handleExportCSV}>
                 <Download className="w-4 h-4" />
                 Export CSV
            </Button>
            <Button className="h-10 gap-2 rounded-xl bg-gray-900 text-white hover:bg-gray-800 shadow-lg shadow-gray-900/20" onClick={handleManageMethods}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm relative overflow-hidden group">
              ...
              <h3 className="text-3xl font-bold text-gray-900">${totalSpent.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</h3>
              <p className="text-xs text-green-600 font-medium mt-2 flex items-center gap-1">
                  <span className="bg-green-100 px-1.5 py-0.5 rounded-md">Lifetime</span> total
              </p>
          </div>
           <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm relative overflow-hidden group">
               <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
           ...
           <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm relative overflow-hidden group">
              ...
              <p className="text-sm font-medium text-gray-500 mb-1">Last Used Method</p>
              <h3 className="text-xl font-bold text-gray-900 truncate">{activeMethod || 'N/A'}</h3>
              <p className="text-xs text-gray-500 font-medium mt-2">
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="p-5 border-b border-gray-100 flex flex-col sm:flex-row gap-4 justify-between items-center bg-gray-50/50">
             ...
             <div className="flex items-center gap-2 w-full sm:w-auto">
                 <Button variant="outline" size="sm" className="h-9 gap-2 rounded-lg border-gray-200 text-gray-600">
        ...
        <div className="overflow-x-auto">
            ...
            <TableBody>
                ...
                <TableRow key={trx.id} className="group hover:bg-gray-50/50 border-gray-100 transition-colors">
                    <TableCell className="font-medium pl-6 text-gray-900">{trx.id}</TableCell>
                    <TableCell className="text-gray-600 font-medium">{trx.description}</TableCell>
                    ...
                    <Badge 
                        ...
                        trx.status === "completed" && "bg-green-50 text-green-700 hover:bg-green-100",
                        trx.status === "pending" && "bg-yellow-50 text-yellow-700 hover:bg-yellow-100",
                        trx.status === "failed" && "bg-red-50 text-red-700 hover:bg-red-100"
                        )}
                    >
