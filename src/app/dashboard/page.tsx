  ...
  return (
    <div className="w-full mx-auto sm:px-2 lg:px-4 py-8 lg:py-16 min-h-[100dvh]">
      <motion.header 
        ...
        className="mb-16 md:mb-24 px-4 flex flex-col md:flex-row md:items-end justify-between gap-8"
      >
        <div>
          ...
          <h1 className="text-4xl md:text-6xl font-bold text-slate-900 tracking-tighter leading-[1.1] mb-4">
            Hello, {user.name?.split(" ")[0] || "User"}.
          </h1>
          <p className="text-lg font-medium text-slate-500 max-w-[40ch] leading-relaxed">
            {t("dashboard.monitorProgress")}
          </p>
        </div>
        
        {/* Right side quick action/status - Asymmetry */}
        <div className="hidden md:flex flex-col items-end">
          <div className="text-right pb-2 border-b border-slate-200/60 w-48">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Active Plan</span>
            <div className="font-semibold text-slate-900 mt-1">Foundational Track</div>
          </div>
        </div>
      </motion.header>
      ...
      >
        ...
        {/* TIER 2: The Identity Cluster (70/30 Horizontal Split) */}

        {/* Left Gallery Widget: PCAResults (Span 5) */}
        <motion.section variants={sectionVariants} className="md:col-span-8 lg:col-span-5 flex flex-col mt-8 lg:mt-12 relative gap-4">
          <PCAResults pcaDataProp={dashboardData?.pcaResults || dashboardData?.PcaResults} />
        </motion.section>
        
        {/* Right Gallery Widget: MILResults (Span 3) */}
        <motion.section variants={sectionVariants} className="md:col-span-8 lg:col-span-3 flex flex-col mt-8 lg:mt-12 relative gap-4">
          <MILResults milDataProp={dashboardData?.milResults || dashboardData?.MilResults} />
        </motion.section>
