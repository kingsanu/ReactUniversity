...
import { ArrowRight, TrendingUp, Users, Globe, BookOpen, DollarSign, Briefcase, Zap, Sparkles, CheckCircle2 } from "lucide-react";
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const [salary, insights, skills, workMode, col, certs, employability] = await Promise.all([
        ...
        setData({ salary, insights, skills, workMode, col, certs, employability });
      } catch (error) {
  ...
  const currentEmployability = data.employability.length > 0 
    ...
    <div className="space-y-6 pb-10">
      ...
      >
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-emerald-100/50 flex items-start justify-between relative overflow-hidden group">
            ...
            <div>
                <p className="text-sm font-medium text-emerald-600 mb-1">Market Demand</p>
                <h3 className="text-2xl font-bold text-gray-900">High</h3>
                <p className="text-xs text-gray-500 mt-2">Top 10% growth rate</p>
            </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-blue-100/50 flex items-start justify-between relative overflow-hidden group">
            ...
            <div>
                <p className="text-sm font-medium text-blue-600 mb-1">Talent Supply</p>
                <h3 className="text-2xl font-bold text-gray-900">Moderate</h3>
                <p className="text-xs text-gray-500 mt-2">Balanced competition</p>
            </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-amber-100/50 flex items-start justify-between relative overflow-hidden group">
            ...
            <div>
                <p className="text-sm font-medium text-amber-600 mb-1">Employability</p>
                <h3 className="text-2xl font-bold text-gray-900">{Math.round(currentEmployability)}%</h3>
      ...
      >
        ...
        <div className="xl:col-span-2 h-full">
           <motion.div variants={item} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all duration-300 group h-full">
             <div className="p-6 h-full flex flex-col">
               <div className="flex flex-row items-center justify-between mb-8">
                  <div className="flex items-center gap-4">
                     ...
                     <div>
                        <h3 className="text-lg font-bold text-gray-900">Salary Benchmarks</h3>
                     </div>
                  </div>
                  <Button variant="ghost" size="sm" asChild className="text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg hidden sm:flex h-8">
                    <Link href={`/dashboard/benchmarks/compensation?country=${country}&career=${career}`}>
        ...
        <div className="xl:col-span-1 h-full">
           <motion.div variants={item} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all duration-300 flex flex-col h-full group">
             <div className="p-6 flex-1 flex flex-col">
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                     ...
                     <Button variant="ghost" size="sm" asChild className="text-gray-400 hover:text-violet-600 hover:bg-violet-50 rounded-lg h-8 px-2">
                        <Link href={`/dashboard/benchmarks/skills?country=${country}&career=${career}`}>
            ...
            <motion.div variants={item} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all duration-300 group h-full">
                <div className="p-6 h-full flex flex-col">
                  ...
                  <div className="flex-1 min-h-[200px]">
                     {data.col && <CostOfLiving data={data.col} isLoading={loading} />}
                  </div>
                </div>
            </motion.div>
        </div>
        ...
        <div className="xl:col-span-1 h-full">
           {/* Quick Certs */}
            <motion.div variants={item} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all duration-300 p-6 h-full flex flex-col">
               <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                     <div className="w-10 h-10 bg-violet-50 rounded-lg flex items-center justify-center">
               ...
               <div className="space-y-4 flex-1">
                   {data.certs.slice(0, 3).map((cert, i) => (
                      <div key={i} className="flex items-start gap-3 group/item cursor-pointer p-3 rounded-xl hover:bg-gray-50 transition-colors border border-transparent hover:border-gray-100">
                          <CheckCircle2 className="w-5 h-5 text-violet-400 mt-0.5 shrink-0 group-hover/item:text-violet-600 transition-colors" />
                          <div>
                              <p className="font-semibold text-sm text-gray-900 group-hover/item:text-violet-700 transition-colors line-clamp-1">{cert.name}</p>
                              <p className="text-xs text-gray-500 mt-1">{cert.provider}</p>
                          </div>
                      </div>
                   ))}
               </div>
               
               <Button variant="ghost" className="w-full mt-4 text-xs text-gray-500 hover:text-violet-600 hover:bg-violet-50 h-9" asChild>
                  <Link href={`/dashboard/benchmarks/skills?country=${country}&career=${career}`}>
                   View All
                 </Link>
               </Button>
        ...
        <div className="xl:col-span-3">
           <motion.div variants={item} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all duration-300">
              <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-500">Real-time industry metrics</p>
                    </div>
                  </div>
