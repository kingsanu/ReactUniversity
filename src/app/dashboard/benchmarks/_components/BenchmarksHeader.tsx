...
export default function BenchmarksHeader({ onExportCSV, onExportImage }: BenchmarksHeaderProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const pathname = usePathname();

  // Initialize state from URL params or defaults
  const [country, setCountry] = useState(searchParams.get("country") || COUNTRIES[0]);
  const [career, setCareer] = useState(searchParams.get("career") || CAREERS[0]);
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-6 mb-10 pb-6 border-b border-slate-200">
        <div className="space-y-2">
          ...
          <p className="text-lg text-slate-500 max-w-2xl font-light">
            Deep-dive into global market trends, salary benchmarks, and skill requirements for your path.
          </p>
        </div>
           ...
           <div className="flex flex-col sm:flex-row gap-3">
             ...
             <div className="relative">
              ...
              <div className="flex gap-2 sm:ml-auto">
                {onExportCSV && (
                  <Button variant="outline" onClick={onExportCSV} size="icon" className="bg-white hover:bg-slate-50 shadow-sm rounded-lg h-10 w-10">
                    <Download className="h-4 w-4 text-slate-600" />
                  </Button>
                )}
                {onExportImage && (
                  <Button onClick={onExportImage} className="bg-slate-900 hover:bg-slate-800 text-white shadow-sm rounded-lg">
