...
export async function GET(request: NextRequest) {
  try {
    ...
    const types = url.searchParams.getAll('types').length > 0
      ? url.searchParams.getAll('types')
      : undefined;
    const status = url.searchParams.getAll('status').length > 0
    ...
    // Get timeline events
    const result = await getTimelineEvents(userId, filters, lang as 'en' | 'sp');
