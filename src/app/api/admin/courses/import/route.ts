...
function isValidProvider(url: string) {
  try {
    ...
    const allowlist = ["coursera.org", "www.coursera.org", "edx.org", "www.edx.org"];
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!url || typeof url !== "string") {
      return NextResponse.json({ success: false, error: { code: "INVALID_REQUEST", message: "Missing url" } }, { status: 400 });
    }

    if (!isValidProvider(url)) {
      return NextResponse.json({ success: false, error: { code: "UNSUPPORTED_DOMAIN", message: "Provider domain not supported for import" } }, { status: 403 });
    }
    ...
    return NextResponse.json({ success: true, jobId: job.jobId, statusUrl: `/api/admin/courses/import/${job.jobId}/status` }, { status: 202 });
  } catch (err) {
