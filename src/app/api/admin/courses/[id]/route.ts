...
import { getAdminCourse, updateAdminCourse, deleteAdminCourse } from "@/lib/adminCoursesStore";
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Course removed successfully" }, { status: 200 });
  } catch (err) {
    return NextResponse.json({ success: false, error: { code: "INTERNAL_ERROR", message: (err as Error).message } }, { status: 500 });
  }
}
